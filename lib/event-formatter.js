const githubEvents = {
    ...
    CommitCommentEvent: ({ name, action, eventAmount }) => {
        return `${_capitalize(action)} ${_aEventQt(eventAmount)} commit ${
            _pluralize(eventAmount, 'comment')
        } at ${name}`;
    ...
    GollumEvent: ({ name, action, eventAmount }) => {
        return `${_capitalize(action)} ${_aEventQt(eventAmount)} wiki ${
            _pluralize(eventAmount, 'page')
        } for ${name}`;
    ...
    IssueCommentEvent: ({ name, action, eventAmount }) => {
        return `${_capitalize(action)} ${_anEventQt(eventAmount)} issue ${
            _pluralize(eventAmount, 'comment')
        } at ${name}`;
    ...
    IssuesEvent: ({ name, action, eventAmount }) => {
        return `${_capitalize(action)} ${_eventQt(eventAmount)} ${
            _issueQt(eventAmount)
        } at ${name}`;
        // name: "an  issue",
        // action: {
        //     opened,
    ...
    PullRequestReviewCommentEvent: ({ name, action, eventAmount }) => {
        return `${_capitalize(action)} ${
            _aEventQt(eventAmount)
        } pull request review ${_pluralize(eventAmount, 'comment')} at ${name}`;
    ...
    PushEvent: ({ name, eventAmount }) => {
        return `Pushed ${_aEventQt(eventAmount)} ${
            _pluralize(eventAmount, 'commit')
        } to ${name}`;
        // "pushed a commit"
    },

    ReleaseEvent: ({ name, action, eventAmount }) => {
        return `${_capitalize(action)} ${_aEventQt(eventAmount)} ${
            _pluralize(eventAmount, 'release')
        } for the repo ${name}`;
