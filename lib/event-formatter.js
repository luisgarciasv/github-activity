const githubEvents = {
    ...
    CommitCommentEvent({ name, action, eventAmount }) {
        return `${this._capitalize(action)} ${this._aEventQt(eventAmount)} commit ${
    ...
    GollumEvent({ name, action, eventAmount }) {
        return `${this._capitalize(action)} ${this._aEventQt(eventAmount)} wiki ${
            this._pluralize(eventAmount, 'page')
        } for ${name}`;
    },

    IssueCommentEvent({ name, action, eventAmount }) {
        return `${this._capitalize(action)} ${this._anEventQt(eventAmount)} issue ${
            this._pluralize(eventAmount, 'comment')
        } at ${name}`;
    },

    IssuesEvent({ name, action, eventAmount }) {
        return `${this._capitalize(action)} ${this._anEventQt(eventAmount)} ${
            this._pluralize(eventAmount, 'issue')
        } at ${name}`;
    },
    ...
    PullRequestEvent({ name, action, merged, eventAmount }) {
        return `${this._capitalize(action)}${merged ? '(merged)' : ''} ${this._aEventQt(eventAmount)} pull ${
            this._pluralize(eventAmount, 'request')
        } at ${name}`;
    },

    PullRequestReviewEvent({ name, action, eventAmount }) {
        return `${this._capitalize(action)} ${this._aEventQt(eventAmount)} pull request ${
            this._pluralize(eventAmount, 'review')
        } at ${name}`;
    },

    PullRequestReviewCommentEvent({ name, action, eventAmount }) {
        return `${this._capitalize(action)} ${this._aEventQt(eventAmount)} pull request review ${
            this._pluralize(eventAmount, 'comment')
        } at ${name}`;
    },

    PushEvent({ name, eventAmount }) {
        return `Pushed ${this._aEventQt(eventAmount)} ${this._pluralize(eventAmount, 'commit')} to ${name}`;
    },
